      ...
      if (React.isValidElement(child)) {
        if (child.props && child.props.field) {
          const column = this.wrapperSortable(
              <HeaderCell minimum {...child.props} width={`${(child.props.width || this.getDefaultWidth())}%`}>
                {child.props.children || child.props.field}
              </HeaderCell>
            , child);
          columns.push(column);
    ...
    React.Children.map(this.props.children, child => {
      if (React.isValidElement(child)) {
        if (child.props && child.props.field) {
          ...
          const cell = (
            <Cell truncate {...child.props} width={`${(child.props.width || this.getDefaultWidth())}%`}>
              {cellValue}
            </Cell>
          );
          cells.push(cell);
    ...
    React.Children.map(this.props.children, child => {
      if (React.isValidElement(child)) {
        if (child.props && child.props.field) {
          ...
          const cell = (
            <DraggableCell truncate {...child.props} isDragging={snapshot.isDragging} width={`${(child.props.width || this.getDefaultWidth())}%`}>
              {cellValue}
            </DraggableCell>
          );
          cells.push(cell);
